...
package cheshire_pkg;
  ...
  localparam cheshire_cfg_t DefaultCfg = '{
    ...
    // Harts
    NumCores          : 1,
    CoreMaxTxns       : `ifdef GENESYS2 8 `elsif VCU108 8 `else 2 `endif,
    CoreMaxTxnsPerId  : `ifdef GENESYS2 8 `elsif VCU108 8 `else 2 `endif,
    ...
    AxiMstIdWidth     : 8,
    AxiMaxMstTrans    : `ifdef GENESYS2 24 `elsif VCU108 24 `else 1 `endif,
    AxiMaxSlvTrans    : `ifdef GENESYS2 24 `elsif VCU108 24 `else 1 `endif,
    AxiUserAmoMsb     : 1, // Convention: lower AMO bits for cores, MSB for serial link
    AxiUserAmoLsb     : 0, // Convention: lower AMO bits for cores, MSB for serial link
    AxiUserErrBits    : 0,
    AxiUserErrLsb     : 0,
    AxiUserDefault    : 0,
    RegMaxReadTxns    : `ifdef GENESYS2 8 `elsif VCU108 8 `else 1 `endif,
    RegMaxWriteTxns   : `ifdef GENESYS2 8 `elsif VCU108 8 `else 1 `endif,
    RegAmoNumCuts     : 1,
    RegAmoPostCut     : 1,
    RegAdaptMemCut    : 1,
    // RTC
    ...
    DbgMaxReqs        : `ifdef GENESYS2 4 `elsif VCU108 4 `else 1 `endif,
    DbgMaxReadTxns    : `ifdef GENESYS2 4 `elsif VCU108 4 `else 1 `endif,
    DbgMaxWriteTxns   : `ifdef GENESYS2 4 `elsif VCU108 4 `else 1 `endif,
    DbgAmoNumCuts     : 1,
    DbgAmoPostCut     : 1,
    ...
    LlcNumLines       : 256,
    LlcNumBlocks      : 8,
    LlcMaxReadTxns    : `ifdef GENESYS2 16 `elsif VCU108 16 `else 1 `endif,
    LlcMaxWriteTxns   : `ifdef GENESYS2 16 `elsif VCU108 16 `else 1 `endif,
    LlcAmoNumCuts     : 1,
    LlcAmoPostCut     : 1,
    LlcOutConnect     : 1,
    LlcOutRegionStart : 'h8000_0000,
    LlcOutRegionEnd   : 64'h1_0000_0000,
    // L2 Cache: shared non-blocking axi_llc that caches BOTH CPU and NVDLA DRAM
    // traffic (single coherency point above the LLC). 8-way, 64 B lines. Size is
    // per-board to respect BRAM budget: VCU108/VCU118 (ample BRAM) get 512 KiB
    // (1024 lines); Genesys2 (xc7k325t, ~445 BRAM shared with NVDLA CBUF) gets a
    // conservative 256 KiB (512 lines). Bump L2NumLines if it fits and you want
    // more NVDLA working-set capacity; drop to 256 (128 KiB) if it does not fit.
    L2Enable          : 1,
    L2SetAssoc        : 8,
    L2NumLines        : `ifdef GENESYS2 512 `elsif VCU108 1024 `elsif VCU118 1024 `else 256 `endif,
    L2NumBlocks       : 8,
