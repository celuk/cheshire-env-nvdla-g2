...
module cache_way #(
    ...
    // Tag and line arrays: BRAM-inferred (no async reset).
    // Valid bits in cache.sv guarantee uninitialized entries are never used.
    (* ram_style = "block" *) logic [TAG_BIT_W-1:0]     tags  [WAY_LEN];
    (* ram_style = "block" *) logic [LINE_BYTE_W*8-1:0] lines [WAY_LEN];

    // Dirty bits: small enough for FFs, needs async reset.
    logic [WAY_LEN-1:0] dirty;

    // BRAM read outputs (before bypass)
    logic [TAG_BIT_W-1:0]     bram_rtag;
    logic [LINE_BYTE_W*8-1:0] bram_rline;
    logic                     bram_rdirty;

    // Tag BRAM: simple dual-port, read-first
    always_ff @(posedge clk_i) begin
        if (we_i)
            tags[windex_i] <= wtag_i;
        bram_rtag <= tags[rindex_i];
    end

    // Line BRAM: simple dual-port with byte enables, read-first
    always_ff @(posedge clk_i) begin
        if (we_i) begin
        ...
        bram_rline <= lines[rindex_i];
    end

    // Dirty bits: FF-based with async reset
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            dirty       <= '0;
            bram_rdirty <= 1'b0;
            ...
            bram_rdirty <= dirty[rindex_i];
        end
    end

    // Read-during-write bypass: BRAM read-first mode returns old data when
    // reading and writing the same index. The cache relies on seeing the
    // just-written value (e.g. after a line fill), so we register the write
    // inputs and MUX them onto the output when an address collision occurred.
    logic                     bypass_q;
    logic [TAG_BIT_W-1:0]     bypass_tag_q;
    logic [LINE_BYTE_W-1:0]   bypass_be_q;
    logic [LINE_BYTE_W*8-1:0] bypass_data_q;
    logic                     bypass_dirty_q;

    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            bypass_q <= 1'b0;
        end else begin
            bypass_q       <= we_i & (rindex_i == windex_i);
            bypass_tag_q   <= wtag_i;
            bypass_be_q    <= wline_be_i;
            bypass_data_q  <= wline_data_i;
            bypass_dirty_q <= wdirty_i;
        end
    end

    // Output MUX: prefer bypass data when a collision was detected
    assign rtag_o   = bypass_q ? bypass_tag_q   : bram_rtag;
    assign rdirty_o = bypass_q ? bypass_dirty_q : bram_rdirty;
    ...
    always_comb begin
        ...
        if (bypass_q) begin
            for (int i = 0; i < LINE_BYTE_W; i++) begin
                if (bypass_be_q[i])
                    rline_o[8*i +: 8] = bypass_data_q[8*i +: 8];
            end
        end
    end
