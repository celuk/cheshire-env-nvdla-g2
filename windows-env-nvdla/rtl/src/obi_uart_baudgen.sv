...
(
  ...
  logic [2:0] dither_cnt;
  logic [15:0] current_limit;

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
       dither_cnt <= '0;
    end else if (oversample_rate_edge_o) begin
       dither_cnt <= dither_cnt + 1;
    end
  end

  always_comb begin
    current_limit = divisor;
    if (divisor == 16'd2) begin
       case(dither_cnt)
           3'd2, 3'd4, 3'd7: current_limit = 16'd3;
           default: current_limit = 16'd2;
       endcase
    end
  end

  assign oversample_is_divisor = (oversample_count == current_limit) & divisor_valid;
